...
Upcoming option to switch between multiple and single window
]
...
... or use GRASS and Python within Jupyter Lab/notebooks via **grass.jupyter**: a new library under development 
...
## .black[Hands-on to space-time analysis for disease ecology with GRASS GIS and] `r fa("r-project", fill="#276dc2", width = "2em")` 
...
.pull-left-wide[
<img src="assets/img/sdm_workflow.png" width="90%">
]
...
- Download and unzip [eu_laea location with LST mapset](https://zenodo.org/record/3564515) and unzip within your `grassdata` folder
- Download asian tiger [mosquito occurrences](https://github.com/veroandreo/grass_opengeohub2021/raw/master/data/aedes_albopictus.gpkg) as `.gpkg` file
- Download [GRASS script](https://github.com/veroandreo/grass_opengeohub2021/raw/master/code/grass_R_disease_ecology_code.sh)
- Download [R script](https://github.com/veroandreo/grass_opengeohub2021/raw/master/code/grass_R_disease_ecology_code.r)
...
.footnote[See [t.create](https://grass.osgeo.org/grass-stable/manuals/t.create.html) and [t.register](https://grass.osgeo.org/grass-stable/manuals/t.register.html)]
...
.panel[.panel-name[Map]
.center[<img src="assets/img/bio1.png" width="51%">]
]
]
...
.panelset[
.panel[.panel-name[R code]
```{r read_vectors, message=FALSE, warning=FALSE, results='hide', eval=FALSE}
# Use sf for vectors
use_sf()
...
```{r data_prep2, message=FALSE, warning=FALSE, results='hide', eval=FALSE}
...
myBiomodOption <- BIOMOD_ModelingOptions(
  MAXENT.Phillips = 
    list(path_to_maxent.jar = "/home/veroandreo/software/maxent/maxent.jar",
         lq2lqptthreshold = 100, l2lqthreshold = 100))
```
...
.panel[.panel-name[Output]
```{r ref.label='vi', warning=FALSE, echo=FALSE}
head(vi[1:13,1, ,], n = 10L) %>% 
  knitr::kable(format = 'html')
```
]
...
.panelset[
.panel[.panel-name[R code]
```{r model_projection, message=FALSE, warning=FALSE, results='hide', eval=FALSE}
...
.panel[.panel-name[Map]
```{r plot_model_proj, message=FALSE, warning=FALSE, echo=FALSE, fig.width=6.5, fig.height=3.5}
# Plot predicted model
plot(mod_proj[[1]])
```
]
...
This is only an example and only the beginning...

- other models to test
- hyper-parameter tuning
- variable selection and model selection
- model averaging or model ensemble
- model validation
- uncertainty: where we can predict with confidence
- many other relevant packages: dismo, sdm, SDMtune, SDMTools, kuenm, caret, CAST, etc.
