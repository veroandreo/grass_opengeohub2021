...
.pull-right-narrow[
<img src="assets/img/temporal_modules.png" width="110%">
]
...
.footnote[See [t.create]() and [t.register]()]
...
.panel[.panel-name[Map]
.center[<img src="assets/img/autumnal_cooling.png" width="50%">]
]
]
...
# Median number of consecutive days with LSTmean <= -2
...
.panel[.panel-name[Output]
```{r ref.label='grass_init', echo=FALSE, size="\tiny"}
# start GRASS GIS from R
initGRASS(gisBase = myGRASS, home = tempdir(), gisDbase = myGISDbase, 
          location = myLocation, mapset = myMapset, override = TRUE)
```
...
<!-- que mas hacer? otros modelos, model averaging, ver las respuestas, ver la incertidumbre,  -->
<!-- ver donde seria factible predecir y donde no, seleccion de variables, etc. -->
<!-- paquetes interesantes y links -->
...
# `r emo::ji("help")` GRASS GIS HELP!!! `r emo::ji("help")`
